...
class wind_turbine():
    ...
    def __init__(self,C_p = 0.42, r = 40, height = 40, capital_cost = 1547, OM_cost = 0.03, V_ci = 4, V_co = 25, V_r = 14, life_time = 20):
        """
    ...
    def rated_power(self):
        return self.wt_ac_output(v_wind=self.V_r/math.pow(self.height / 10, 0.16))

    def get_v_wind(self, power):
        '''
        此函数用于在已知功率的情况下反算速度，并不属于风机自身特性。属于方便开发者使用的工具。
    ...
    def get_fixed_cost(self):
        # 获取固定成本
        # 注意输入的成本单位为$/kW,需转化为$/MW
        return self.rated_power()*self.capital_cost*1000
